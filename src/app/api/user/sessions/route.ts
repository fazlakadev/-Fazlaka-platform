...
export async function GET(request: NextRequest) {
  ...
  const geoMap = new Map<string, { country: string; city: string; region: string; flag: string; lat: number; lng: number; postal: string; isp: string; org: string; timezone: string; continent: string } | null>()
  await Promise.all(
  ...
  const sessions: (PublicSession & { isCurrent: boolean })[] = rows.map((r) => {
    ...
    const pub: PublicSession & { isCurrent: boolean } = {
      ...
      location: (r.ip ? geoMap.get(r.ip) ?? null : null) as { country: string; city: string; region: string; flag: string; lat: number; lng: number; postal: string; isp: string; org: string; timezone: string; continent: string } | null,
