...
import { Eye, Heart, MessageCircle, ArrowLeft, Clock, User, AlertCircle, Trash2, Edit, Calendar, ThumbsUp, ExternalLink, Search, Users, Ban, MailWarning, Loader2, FileText } from 'lucide-react';
...
export default function ArticleAnalyticsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/admin/articles/${slug}/analytics`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900" dir="ltr">
      <div className="max-w-7xl mx-auto p-4 md:p-8">
        <Link href="/admin/articles" className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm mb-8 w-fit">
          <ArrowLeft size={18} /> Back to Articles
        </Link>

        {/* Hero Card */}
        <div className="relative bg-gradient-to-br from-gray-800/80 via-gray-800/40 to-green-900/30 rounded-3xl border border-gray-700/50 overflow-hidden mb-8">
          ...
          <div className="relative p-6 md:p-8 flex flex-col md:flex-row gap-6 items-start">
            <div className="shrink-0 w-full md:w-48 h-48 rounded-2xl overflow-hidden bg-gray-700 border border-gray-600/50 shadow-2xl">
              {thumbnail ? (
                ...
                <div className="w-full h-full flex items-center justify-center"><FileText className="w-12 h-12 text-gray-500" /></div>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex gap-2 shrink-0">
                  <Link href={`/admin/articles/${data.content.slug}/edit`} className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-all text-sm font-semibold shadow-lg shadow-indigo-600/30">
                    <Edit size={16} /> Edit
                  </Link>
                  <a href={`/articles/${data.content.slug}`} target="_blank" className="flex items-center gap-2 px-4 py-2.5 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-xl transition-all text-sm font-semibold">
